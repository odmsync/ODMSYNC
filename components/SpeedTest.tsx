...
import { getSpeedTestRecommendation } from '../services/geminiService';
import type { SpeedTestRecommendation } from '../types';
...
const SpeedTest: React.FC = () => {
  ...
  const [aiRecommendation, setAiRecommendation] = useState<SpeedTestRecommendation | null>(null);
  ...
  // Fetch AI recommendation when test completes
  useEffect(() => {
    if (status === 'completed' && download > 0 && !aiRecommendation && !loadingRecommendation) {
      fetchAIRecommendation();
    }
  }, [status, download, upload, ping, jitter]);
  ...
  const fetchAIRecommendation = async () => {
    ...
    try {
      ...
      const recommendation = await getSpeedTestRecommendation(
        download,
        upload,
        ping,
        jitter,
      ...
      analytics.track('ai_recommendation_generated', {
        plan: recommendation.recommendedPlanId,
  ...
  const recommendedPlan = aiRecommendation 
    ? getPlans(language).find(p => p.id === aiRecommendation.recommendedPlanId) || getRecommendation()
    : getRecommendation();
        ...
        <div className="bg-gray-800/80 backdrop-blur-lg rounded-2xl md:rounded-3xl shadow-2xl border border-gray-700 p-5 md:p-12">
          ...
          {status === 'completed' && (
            <div className="animate-fade-in-up space-y-6 md:space-y-8 border-t border-gray-700 pt-6 md:pt-8 mt-6 md:mt-8">
              ...
              {status === 'completed' && (aiRecommendation || recommendedPlan) && (
                <div className="bg-gradient-to-r from-blue-900/50 to-teal-900/50 border border-blue-500/30 rounded-2xl p-5 md:p-6 flex flex-col md:flex-row items-center justify-between gap-6 shadow-lg">
                  <div className="flex-1 text-center md:text-start">
                    ...
                    <h3 className="text-2xl font-bold text-white mb-2">
                      {loadingRecommendation 
                        ? (language === 'ar' ? 'جاري التوصية...' : 'Getting recommendation...')
                    ...
                    {!loadingRecommendation && (
                      <p className="text-gray-300 text-sm mb-4 md:mb-0">
                        {aiRecommendation 
                          ? (language === 'ar' ? aiRecommendation.reason.ar : aiRecommendation.reason.en)
                  ...
                  {!loadingRecommendation && recommendedPlan && (
                    <div className="flex flex-col items-center md:items-end gap-2 w-full md:w-auto">
                      <div className="text-2xl font-bold text-white">
                        {recommendedPlan.priceUSD}/{t.plans.per_mo}
                      </div>
                      <div className="text-xs text-gray-400 line-through">
                        {t.speedtest.competitor}: $20+
                      </div>
                      <a 
                        ...
                        target="_blank"
                        rel="noreferrer"
                        onClick={() => analytics.track('plan_selected', { 
                          plan: recommendedPlan.name, 
                          source: aiRecommendation ? 'speed_test_ai_recommendation' : 'speed_test_recommendation', 
                          language,
